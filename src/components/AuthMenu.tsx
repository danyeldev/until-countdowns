...
export function AuthMenu() {
  ...
  if (account === undefined) {
    return <div className="h-11 w-[4.75rem] rounded-xl bg-surface" aria-hidden="true" />;
  }

  if (!account) {
    return (
      <SignInButton className="button-secondary !min-h-11 !px-4 !text-sm">
        Sign in
      </SignInButton>
    );
  }
  ...
  return (
    <details className="relative">
      <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-xl py-1.5 pl-1.5 pr-3 text-sm text-paper-dim transition hover:bg-surface hover:text-paper [&::-webkit-details-marker]:hidden">
        <span className="flex size-8 items-center justify-center rounded-full bg-amber/15 text-sm font-medium text-amber">
          {initialFor(account)}
        </span>
        <span className="hidden max-w-[10rem] truncate sm:inline">{account.label}</span>
        <span className="sr-only sm:hidden">Account</span>
      </summary>
      <div className="menu absolute end-0 z-50 mt-2 w-64 !p-3">
