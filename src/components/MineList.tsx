    ...
    <section className="mt-10 border-t border-line pt-8" aria-label="Your recent countdowns">
      ...
      <ul className="panel mt-5 divide-y divide-line overflow-hidden">{mine.slice(0, 3).map((event) => (
        <li key={event.id} className="flex items-center justify-between gap-3 px-4 py-3 sm:px-5">
          ...
          <button type="button" aria-label={`Remove ${event.title}`} onClick={() => { try { removeMine(event.id); setError(""); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not remove this countdown."); } }} className="min-h-11 shrink-0 rounded-xl px-3 text-xs text-muted hover:bg-ink hover:text-paper">Remove</button>
        </li>
      ))}</ul>
