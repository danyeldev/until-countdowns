...
- **What the metadata says** — verified through Commons `imageinfo&iiprop=url|size|mime|extmetadata` (**never** `en.wikipedia.org`, which returns fair-use files with a working URL). Accepted: CC0, public domain / PD, CC BY, CC BY-SA, GODL-India, OGL, KOGL, NASA media guidelines, the Flickr Commons labels "No restrictions" and "Attribution", and the free software licences logos and screenshots carry (Apache, MIT, BSD, GPL, MPL, OFL). Rejected: `NonFree=true`, "Fair use", any NC or ND clause, `Restrictions` naming currency, a required attribution that names nobody ("Multiple authors" on a montage), and **a missing licence name** — freedom is never assumed. The `trademarked`, `insignia` and `personality` restriction tags are accepted: a product's own logo on its end-of-life page, a coat of arms on an election page and a photo of the athlete or politician an event is about are referential / editorial use, and refusing them left every election, most sports rows and all software end-of-life rows without a picture. Any mime type sharp can rasterise is accepted — JPEG, PNG, WebP, GIF, TIFF and SVG (rendered at hero width first).
